...
type Props = NativeStackScreenProps<FreelancerStackParamList, 'FreelancerProjects'> & {
    searchQuery?: string;
};
...
export default function FreelancerProjectsScreen({ navigation, searchQuery = '' }: Props) {
    const { user } = userAuthStore();

    const { data: projects, isLoading } = useQuery({
        queryKey: ['freelancerProjects', user?.userId],
        queryFn: async () => {
            console.log('[FreelancerProjects] Fetching projects for:', user?.userId);
            const data = await getAllProjectsForFreelancer(user!.userId);
            console.log('[FreelancerProjects] Projects data:', JSON.stringify(data, null, 2));
            return data;
        },
        enabled: !!user?.userId,
    });
    ...
    const filteredProjects = useMemo(() => {
        if (!projects) {
            console.log('[FreelancerProjects] No projects data');
            return [];
        }
        
        console.log('[FreelancerProjects] Total projects:', projects.length);
        console.log('[FreelancerProjects] Projects with data:', projects.filter(p => p.project != null).length);
        
        // Filter out null projects
        const validProjects = projects.filter(p => p.project != null);
        
        if (!searchQuery.trim()) {
            console.log('[FreelancerProjects] No search query, returning all valid projects');
            return validProjects;
        }
        
        const query = searchQuery.toLowerCase();
        const filtered = validProjects.filter((item) => {
        ...
        console.log('[FreelancerProjects] After search filter:', filtered.length);
        return filtered;
    ...
    if (!projects || projects.length === 0) {
        return (
            <View style={styles.container}>
                <Empty
                    title="No projects"
                    description="You haven't joined any projects yet. Check your invitations!"
                />
            </View>
        );
    }
    ...
    return (
        <View style={styles.container}>
            <FlatList
                ...
                keyExtractor={(item, index) => `${item.project?.id || index}-${index}`}
                            ...
                            <View style={styles.cardContent}>
                                ...
                                <View style={styles.createdAtRow}>
                                    ...
                                    <Text style={styles.createdAtText}>
                                        Created {new Date(item.project?.created_at || Date.now()).toLocaleDateString('en-GB', {
                                            day: 'numeric',
                                            month: 'short',
                                            year: 'numeric',
                                        })}
                                ...
                                {/* Domains */}
                                {item.project?.domains && item.project?.domains.length > 0 && (
                                    <View style={styles.domainsRow}>
                                        {item.project.domains.slice(0, 2).map((domain, idx) => (
                                        ...
                                        {item.project.domains.length > 2 && (
                                            <View style={[styles.domainChip, styles.moreDomainChip]}>
                                                <Text style={styles.moreDomainText}>+{item.project.domains.length - 2}</Text>
                                            </View>
                                        )}
                                    </View>
                                )}
                                ...
                                <View style={styles.footerRow}>
                                    <View style={styles.budgetContainer}>
                                        ...
                                        <Text style={styles.budgetAmount}>Rs. {item.project?.budget || 0}</Text>
                                    </View>
